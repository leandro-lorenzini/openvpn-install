...
crl-verify crl.pem" >> /etc/openvpn/server/server.conf
	...
	if systemctl is-active --quiet firewalld.service; then
		...
		firewall-cmd --add-port=11122/"$protocol"
		firewall-cmd --zone=trusted --add-source=10.8.0.0/24
		firewall-cmd --zone=trusted --add-source=10.9.0.0/24
		firewall-cmd --permanent --add-port="$port"/"$protocol"
		firewall-cmd --permanent --add-port=11122/"$protocol"
...
ExecStart=$iptables_path -I INPUT -p $protocol --dport 11122 -j ACCEPT
...
ExecStop=$iptables_path -D INPUT -p $protocol --dport 11122 -j ACCEPT
		...
		3)
			...
			if [[ "$remove" =~ ^[yY]$ ]]; then
				...
				if systemctl is-active --quiet firewalld.service; then
					...
					firewall-cmd --remove-port=11122/"$protocol"
					firewall-cmd --zone=trusted --remove-source=10.8.0.0/24
					firewall-cmd --zone=trusted --remove-source=10.9.0.0/24
					firewall-cmd --permanent --remove-port="$port"/"$protocol"
					firewall-cmd --permanent --remove-port=11122/"$protocol"
